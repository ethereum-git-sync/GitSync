...
function gather_issues() {
	...
	gh issue -s all -L 100000 --json number,title,author,updatedAt list | jq -r 'sort_by(.updatedAt) | reverse' > /home/ubuntu/GitSync/archive/${1}/issue/issues_current.json
	echo "The updated list was collected"

	#Generate a text file containing the number of each issue
	jq -r '.[].number' /home/ubuntu/GitSync/archive/${1}/issue/issues_current.json > /home/ubuntu/GitSync/archive/${1}/issue/index.txt
	...
	#Iterate through the text file to compare the time stamps at each issue with those currently archived
        FILE="/home/ubuntu/GitSync/archive/${1}/issue/index.txt"
        INPUT=$(cat $FILE)
        for LINE in $INPUT
        do
		...
		then
			UPDATED_TIME=$(jq -r --argjson ISSUE_NUMBER "$ISSUE_NUMBER" '.[] | select(.number==$ISSUE_NUMBER) | .updatedAt' \
				/home/ubuntu/GitSync/archive/$1/issue/issues_current.json)
			CURRENT_TIME=$(jq -r --argjson ISSUE_NUMBER "$ISSUE_NUMBER" '.[] | select(.number==$ISSUE_NUMBER) | .updatedAt' \
				/home/ubuntu/GitSync/archive/$1/issue/issues.json)
			...
			#If they are different, then then json file containing the issue must be updated in the archive.
				...
				curl --silent -H "Authorization: token ${PAT}" https://api.github.com/repos/${1}/issues/$ISSUE_NUMBER > /home/ubuntu/GitSync/archive/${1}/issue/"$LINE".json
				curl --silent -H "Authorization: token ${PAT}" https://api.github.com/repos/${1}/issues/$ISSUE_NUMBER/comments \
					>> /home/ubuntu/GitSync/archive/${1}/issue/"$LINE".json
				gh_api_limit_check "/home/ubuntu/GitSync/archive/${1}/issue/"$LINE".json"
			fi
		else
			jq -r --argjson ISSUE_NUMBER "$ISSUE_NUMBER" 'del(.[] | select(.number==$ISSUE_NUMBER))' /home/ubuntu/GitSync/archive/${1}/issue/issues_current.json \
				> /home/ubuntu/GitSync/archive/${1}/issue/temp.json
			cp /home/ubuntu/GitSync/archive/${1}/issue/temp.json /home/ubuntu/GitSync/archive/${1}/issue/issues_current.json
			rm /home/ubuntu/GitSync/archive/${1}/issue/temp.json
